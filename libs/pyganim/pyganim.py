...
# Edited by Marcel Schnelle:
# Changed internals of PygAnimation objects to be usable in a TiledTMXLoader context.
...
class PygAnimation(object):
    def __init__(self, frames, loop=True):
        ...
        #       (image_of_frame<tiledtmxloader.helperspygame.SpriteLayer.Sprite>, duration_in_seconds<int>)
        ...
        # _images stores the Sprite objects of each frame
        self._images = []
        ...
        if frames != '_copy': # ('_copy' is passed for frames by the getCopies() method)
            ...
            for i in range(self.numFrames):
                ...
                assert type(frame[0]) in (str, tiledtmxloader.helperspygame.SpriteLayer.Sprite), 'Frame %s image must be a string filename or a TiledTMXSprite' % (i)
                assert frame[1] > 0, 'Frame %s duration must be greater than zero.' % (i)
                if type(frame[0]) == str:
                    surf = pygame.image.load(frame[0])
                    tmxsprite = tiledtmxloader.helperspygame.SpriteLayer.Sprite(surf, surf.get_rect())
                    frame = (tmxsprite, frame[1])
    ...
    def blit(self, destSurface, dest):
        ...
        destSurface.blit(self.getFrame(frameNum).image, dest)
    ...
    def flip(self, xbool, ybool):
        ...
        for i in range(len(self._images)):
            self._transformedImages[i] = pygame.transform.flip(self.getFrame(i).image, xbool, ybool)
    ...
    def scale(self, width_height):
        ...
        self._makeTransformedSurfacesIfNeeded()
        for i in range(len(self._images)):
            self._transformedImages[i] = pygame.transform.scale(self.getFrame(i).image, width_height)
    ...
    def rotate(self, angle):
        ...
        self._makeTransformedSurfacesIfNeeded()
        for i in range(len(self._images)):
            self._transformedImages[i] = pygame.transform.rotate(self.getFrame(i).image, angle)
    ...
    def rotozoom(self, angle, scale):
        ...
        self._makeTransformedSurfacesIfNeeded()
        for i in range(len(self._images)):
            self._transformedImages[i] = pygame.transform.rotozoom(self.getFrame(i).image, angle, scale)
    ...
    def scale2x(self):
        ...
        self._makeTransformedSurfacesIfNeeded()
        for i in range(len(self._images)):
            self._transformedImages[i] = pygame.transform.scale2x(self.getFrame(i).image)
    ...
    def smoothscale(self, width_height):
        ...
        self._makeTransformedSurfacesIfNeeded()
        for i in range(len(self._images)):
            self._transformedImages[i] = pygame.transform.smoothscale(self.getFrame(i).image, width_height)
    ...
    def _surfaceMethodWrapper(self, wrappedMethodName, *args, **kwargs):
        self._makeTransformedSurfacesIfNeeded()
        for i in range(len(self._images)):
            methodToCall = getattr(self._transformedImages[i].image, wrappedMethodName)
